function FreeboardModel(datasourcePlugins, widgetPlugins, freeboardUI)
{
	...
	this.saveDashboardClicked = function(_thisref, event){
		if(window.agentFreeboardSaveEndpoint)
		{
			$.ajax({
				...
				success: function(){
					alert("Dashboard saved to project.");
				},
				error: function(xhr){
		...
		self.downloadDashboard(true);
	}

	this.downloadDashboard = function(pretty)
	{
		var contentType = 'application/octet-stream';
		var a = document.createElement('a');
		var body = pretty ? JSON.stringify(self.serialize(), null, '\t') : JSON.stringify(self.serialize());
	...
	{
		self.downloadDashboard($(event.currentTarget).data('pretty'));
	}
