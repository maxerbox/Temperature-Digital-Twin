...
(function () {
	var hfDatasource = function (settings, updateCallback) {
		...
		this.updateNow = function () {
			...
			var dataset = currentSettings.dataset || "maxerbox/temperature_digital_twin";
			...
			function fetchBatch() {
				var url = baseUrl + "?dataset=" + encodeURIComponent(dataset) +
					"&config=" + encodeURIComponent(config) +
					"&split=" + encodeURIComponent(split) +
					"&offset=" + offset + "&length=" + batchSize;
			...
			function processData(rows) {
				...
				var rawData = rows.map(function (r) { return r.row; });
	...
	freeboard.loadDatasourcePlugin({
		...
		description: "Fetches sensor data from HuggingFace Dataset Viewer API with date filtering and per-sensor organization.",
		settings: [
			{
				name: "dataset",
			...
			},
			{
				...
				description: "Filter data by this date (YYYY-MM-DD). Use 'today' for today's data. Leave blank for today.",
				type: "text",
				default_value: "today"
			},
			{
